...
const PAGE_SIZE_KEY = "yv_table_page_size:institution_submissions";
...
function InstitutionSubmissionsContent() {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      const res = await fetch(`/api/institutions/submissions?${params}`);
      if (!res.ok) {
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 md:mt-2 text-sm md:text-base">
            Create and manage compliance packs, readiness reports, and other submissions for QCTO
          </p>
        </div>
        <div className="flex flex-col gap-2 sm:flex-row">
          <ExportButton exportUrl="/api/export/submissions" className="w-full sm:w-auto" />
          <Link href="/institution/submissions/new" className="w-full sm:w-auto">
            <Button className="w-full sm:w-auto">Create Submission</Button>
          </Link>
        </div>
      </div>
              ...
              <EmptyState
                ...
                description={
                  hasActiveFilters
                    ...
                    : "Create your first submission to submit compliance packs for QCTO review."
                }
                action={!hasActiveFilters ? { label: "Create Your First Submission", href: "/institution/submissions/new" } : undefined}
...
export default function InstitutionSubmissionsPage() {
  return (
    <Suspense fallback={<LoadingTable columns={8} rows={6} />}>
      <InstitutionSubmissionsContent />
    </Suspense>
  );
}
