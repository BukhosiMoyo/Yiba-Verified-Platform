...
import { Mail, Plus, Copy, Check, Loader2, Pencil, Search, X, RefreshCw } from "lucide-react";
...
export default function InstitutionInvitesPage() {
  ...
  const canResend = (inv: any) =>
    ...
    ["FAILED", "RETRYING", "SENT", "DECLINED"].includes(inv.status || "");

  const handleResend = async (inviteId: string) => {
    try {
      setResendingId(inviteId);
      const res = await fetch(`/api/institution/invites/${inviteId}/resend`, { method: "POST" });
  ...
  return (
    <div className="space-y-6 md:space-y-8 p-4 md:p-8">
      {/* Header */}
      <div className="rounded-2xl border border-border bg-card px-6 py-6 md:px-8 md:py-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-foreground">Invites</h1>
        ...
        <div className="border-b border-border bg-muted/30 px-4 py-4 md:px-6 md:py-5">
          <div className="flex flex-wrap items-end gap-3">
            <div className="flex-1 min-w-[180px] max-w-xs">
              <Label className="text-muted-foreground text-xs font-medium mb-1 block">Search by email</Label>
              <div className="relative flex gap-2">
                ...
                <Button type="button" variant="outline" size="sm" onClick={applySearch} className="h-10 shrink-0">
                  Search
                </Button>
              </div>
            </div>
            ...
            <div>
              <Label className="text-muted-foreground text-xs font-medium mb-1 block">Role</Label>
              <Select
                value={roleFilter}
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-b border-border hover:bg-transparent">
                      <TableHead className="w-12 text-center h-11 bg-muted/50 font-semibold text-muted-foreground">#</TableHead>
                      <TableHead className="h-11 bg-muted/50 font-semibold text-muted-foreground">Email</TableHead>
                      <TableHead className="h-11 bg-muted/50 font-semibold text-muted-foreground">Role</TableHead>
                      <TableHead className="h-11 bg-muted/50 font-semibold text-muted-foreground">Status</TableHead>
                      <TableHead className="h-11 bg-muted/50 font-semibold text-muted-foreground">Expires</TableHead>
                      <TableHead className="h-11 bg-muted/50 font-semibold text-muted-foreground">Created By</TableHead>
                      <TableHead className="h-11 bg-muted/50 font-semibold text-muted-foreground">Created</TableHead>
                      <TableHead className="sticky right-0 z-10 w-24 min-w-[6rem] h-11 bg-muted/50 font-semibold text-muted-foreground text-right border-l border-border">Actions</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {invites.map((invite, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell className="sticky right-0 z-10 bg-card group-hover:bg-accent/50 border-l border-border text-right py-4">
          ...
          {createdInvite ? (
            <>
              ...
              <div className="space-y-4 py-4">
                <div className="rounded-xl p-4 space-y-2 border border-border bg-muted/50">
                  <div>
                    <p className="text-xs font-medium text-muted-foreground">Email</p>
                    <p className="text-sm text-foreground font-medium">{createdInvite.email}</p>
                  </div>
                  <div>
